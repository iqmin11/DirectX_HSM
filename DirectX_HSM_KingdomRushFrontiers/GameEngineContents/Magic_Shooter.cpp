...
void Magic_Shooter::Start()
{
	...
	BaseShooterRenderer->CreateAnimation({ .AnimationName = "4_Attack_Down", .SpriteName = "ArchMage_Shooter_Attack_Down",.Loop = false });
	BaseShooterRenderer->CreateAnimation({ .AnimationName = "4_Attack_Up", .SpriteName = "ArchMage_Shooter_Attack_Up",.Loop = false });
	BaseShooterRenderer->CreateAnimation({ .AnimationName = "4_Idle_Down", .SpriteName = "ArchMage_Shooter_Idle_Down",.Loop = false });
	BaseShooterRenderer->CreateAnimation({ .AnimationName = "4_Idle_Up", .SpriteName = "ArchMage_Shooter_Idle_Up",.Loop = false });
	...
	BaseShooterRenderer->SetAnimationStartEvent("4_Attack_Down", 4, std::bind(&Magic_Shooter::ArchAttack, this));
	BaseShooterRenderer->SetAnimationStartEvent("4_Attack_Up", 4, std::bind(&Magic_Shooter::ArchAttack, this));
